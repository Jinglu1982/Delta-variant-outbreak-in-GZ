...
rule all:
    input:
        expand("3.mapping/{sample}.bam",sample=SAMPLE_LIST),
...
rule cutadapt:
    ...
    shell:
        """
		bwa mem -t 20 -a {ref} {input.seq1} {input.seq2} | samtools view -@ 4 -bS -F 4 | samtools sort -@ 4 -o {output.bamfile};
		ivar trim -b {primer} -p {params.prefix} -i {output.bamfile} -q 15 -s 4;
        bedtools bamtofastq -i {params.prefix}.bam -fq {output.cutseq1} -fq2 {output.cutseq2};
        rm {params.prefix}.bam
        """
rule mapping:
    input:
...
rule mpileup:
    ...
    shell:
        "samtools mpileup -Q 20 -d 100000 --reference {ref} {input.bamfile} | ivar variants -r {ref} -p {output.snptab} -t 0;"
        "bcftools mpileup --threads 2 -Q 20 -a FORMAT/DP,FORMAT/AD,INFO/AD -I -d 100000 -f {ref} {input.bamfile} > {output.mpileuptab};"
